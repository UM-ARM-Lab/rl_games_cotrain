...
class CentralValueTrain(nn.Module):
    def __init__(
        ...
        self.train_num_actors = train_num_actors if train_num_actors is not None else num_actors
        ...
        if self.linear_lr:
            self.scheduler = schedulers.LinearScheduler(
                self.lr, max_steps=self.max_epochs, apply_to_entropy=False, start_entropy_coef=0
            )
        else:
        ...
        self.minibatch_size = self.config.get("minibatch_size", self.train_num_actors * self.minibatch_size_per_env)
        self.num_minibatches = self.horizon_length * self.train_num_actors // self.minibatch_size
        self.clip_value = config["clip_value"]

        self.writter = writter
        self.weight_decay = config.get("weight_decay", 0.0)
        self.optimizer = torch.optim.Adam(
            self.model.parameters(), float(self.lr), eps=1e-08, weight_decay=self.weight_decay
        )
        self.frame = 0
        self.epoch_num = 0
        ...
        self.batch_size = self.horizon_length * self.train_num_actors

        if self.is_rnn:
            self.rnn_states = self.model.get_default_rnn_state()
            self.rnn_states = [s.to(self.ppo_device) for s in self.rnn_states]
            total_agents = self.train_num_actors  # * self.num_agents
        ...
        self.dataset = datasets.PPODataset(
            self.batch_size, self.minibatch_size, True, self.is_rnn, self.ppo_device, self.seq_length
        )
    ...
    def pre_step_rnn(self, n):
        ...
        if n % self.seq_length == 0:
            for s, mb_s in zip(self.rnn_states, self.mb_rnn_states):
                mb_s[n // self.seq_length, :, :, :] = s[:, : self.train_num_actors, :]
    ...
    def get_value(self, input_dict):
        ...
        res_dict = self.forward(
            {"obs": obs_batch, "actions": actions, "rnn_states": self.rnn_states, "is_train": False}
        )
    ...
    def update_multiagent_tensors(self, value_preds, returns, actions, dones):
        ...
        ma_batch_size = self.train_num_actors * self.num_agents * self.horizon_length
        value_preds = value_preds.view(
            self.train_num_actors, self.num_agents, self.horizon_length, self.value_size
        ).transpose(0, 1)
        returns = returns.view(self.train_num_actors, self.num_agents, self.horizon_length, self.value_size).transpose(
            0, 1
        )
    ...
    def calc_gradients(self, batch):
        ...
        batch_dict = {"obs": obs_batch, "actions": actions_batch, "seq_length": self.seq_length, "dones": dones_batch}
        if self.is_rnn:
        ...
        loss = common_losses.critic_loss(
            self.model, value_preds_batch, values, self.e_clip, returns_batch, self.clip_value
        )
