...
from rl_games.common import common_losses, schedulers, vecenv
...
def print_statistics(
    print_stats, curr_frames, step_time, step_inference_time, total_time, epoch_num, max_epochs, frame, max_frames
):
    if print_stats:
...
class A2CBase(BaseAlgorithm):
    def __init__(self, base_name, params):
        ...
        self.obs = None
        self.num_train_for_minibatch = (
            self.config["num_train_envs"] if self.config.get("train_val", False) else self.num_actors
        )
        self.batch_size = self.horizon_length * self.num_train_for_minibatch * self.num_agents
        self.batch_size_envs = self.horizon_length * self.num_train_for_minibatch
        ...
        self.minibatch_size = self.config.get(
            "minibatch_size", self.num_train_for_minibatch * self.minibatch_size_per_env
        )
        ...
        if self.env_has_train_val:
            ...
            assert self.num_train + self.num_val == self.num_actors
        else:
    ...
    def write_stats(
        ...
        # for key, value in self._collapse_stats.items():
    ...
    def get_action_values(self, obs):
        ...
        self.model.eval()
        input_dict = {"is_train": False, "prev_actions": None, "obs": processed_obs, "rnn_states": self.rnn_states}
    ...
    def init_tensors(self):
        ...
        # OLD experience buffer if not cotrain_enabled
        if not self.cotrain_enabled:
            ...
            self.experience_buffer = CotrainExperienceBuffer(
                ...
                real_env_idx=self.cotrain_cfg.get("real_env_idx", None),
                scoring_mode=self.cotrain_cfg.get("scoring_mode", "binary"),
                temperature=self.cotrain_cfg.get("temperature", None),
                score_direction=self.cotrain_cfg.get("score_direction", "le"),
        ...
        if self.is_rnn:
            ...
            total_agents = self.num_agents * self.num_train_for_minibatch
    ...
    def discount_values_masks(
        self, fdones, last_extrinsic_values, mb_fdones, mb_extrinsic_values, mb_rewards, mb_masks
    ):
        lastgaelam = 0
        mb_advs = torch.zeros_like(mb_rewards)
        for t in reversed(range(self.horizon_length)):
    ...
    def play_steps_rnn(self):
        ...
        train_agent_count = self.num_agents * self.num_train_for_minibatch
        step_time = 0.0

        for n in trange(self.horizon_length, leave=False, desc="Playing steps"):
            if n % self.seq_length == 0:
                for s, mb_s in zip(self.rnn_states, mb_rnn_states):
                    assert s.size(1) >= train_agent_count
                    mb_s[n // self.seq_length, :, :, :] = s[:, :train_agent_count, :]
...
class DiscreteA2CBase(A2CBase):
    ...
    def train(self):
        ...
        while True:
            ...
            step_time, play_time, update_time, sum_time, a_losses, c_losses, entropies, kls, last_lr, lr_mul = (
                self.train_epoch()
            )
                ...
                if self.game_rewards.current_size > 0:
                    ...
                    for i in range(self.value_size):
                        ...
                        self.writer.add_scalar(
                            "shaped_" + rewards_name + "/step".format(i), mean_shaped_rewards[i], frame
                        )
                        self.writer.add_scalar(
                            "shaped_" + rewards_name + "/iter".format(i), mean_shaped_rewards[i], epoch_num
                        )
                        self.writer.add_scalar(
                            "shaped_" + rewards_name + "/time".format(i), mean_shaped_rewards[i], total_time
                        )
